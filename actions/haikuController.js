...
import { justDecodeCookie } from "../lib/getUser.js"
...
export const deleteHaiku = async function (formData) {
  const user = await justDecodeCookie()
  ...
  let idToDelete = formData.get("id")
  if (typeof idToDelete !== "string") idToDelete = ""

  // check that logged in user equals the author of the post in question
  const haikuInQuestion = await haikusCollection.findOne({ _id: ObjectId.createFromHexString(idToDelete) })

  if (haikuInQuestion.author.toString() !== user.userId) {
    return redirect("/")
  }

  await haikusCollection.deleteOne({ _id: ObjectId.createFromHexString(idToDelete) })
  redirect("/")
}
...
async function sharedHaikuLogic(formData, user) {
  ...
  if (!isAlphanumericWithSpaces(ourHaiku.line1)) errors.line1 = "Can only contain letters, numbers, and spaces."
  if (!isAlphanumericWithSpaces(ourHaiku.line2)) errors.line2 = "Can only contain letters, numbers, and spaces."
  if (!isAlphanumericWithSpaces(ourHaiku.line3)) errors.line3 = "Can only contain letters, numbers, and spaces."
  ...
  // verify signature and if its good then add it to ourHaiku object
...
export const updateHaiku = async function (prevState, formData) {
  ...
  // otherwise make update in database
  const haikusCollection = await getCollection("haikus")

  let haikuId = formData.get("haikuId")
  if (typeof haikuId != "string") haikuId = ""

  // cancel if the logged in user is NOT the author of the post in question
